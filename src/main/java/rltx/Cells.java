...
final class Cells
{
	...
	final int[] bits = new int[Renderer.CELL_LAYERS * Renderer.CELL_WORDS];

	void clear(int layer)
	{
		Arrays.fill(bits, layer * Renderer.CELL_WORDS, (layer + 1) * Renderer.CELL_WORDS, 0);
	}
	...
	void mark(int layer, float minX, float minZ, float maxX, float maxZ)
	{
		...
		for (int x = x0; x <= x1; ++x)
		{
			...
			{
				int bit = x * 64 + z;
				bits[layer * Renderer.CELL_WORDS + (bit >> 5)] |= 1 << (bit & 31);
			}
		}
	}
}
