...
public final class NormalRenderer implements Renderer
{
	...
	private long descriptorSetLayout, descriptorPool, dynamicDescriptorSet, staticDescriptorSet, waterDescriptorSet;
	...
	// Which recorded groups are water: drawn by the water pass, not the opaque one. groupZone lets the
	// water pass skip a zone whose static water Waves has replaced with a displaced dynamic copy.
	...
	private void createDescriptors()
	{
		try (MemoryStack stack = stackPush())
		{
			...
			// Three sets now: opaque dynamic, static (opaque and water share it), and dynamic water.
			...
			writeSet(stack, waterDescriptorSet, waterPositions, waterColors, waterUvs, waterTexs);
		}
	}
	...
	private void uploadTextureArray(int layers, int size, int levels, ByteBuffer rgba)
	{
		...
		try (MemoryStack stack = stackPush())
		{
			...
			writes.get(2).sType$Default().dstSet(waterDescriptorSet).dstBinding(4).descriptorType(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER).descriptorCount(1).pImageInfo(imgInfo);
			vkUpdateDescriptorSets(device, writes, null);
		}
	}
		...
		for (Map.Entry<Integer, StaticScene> entry : staticScenes.entrySet())
		{
			...
			for (int zi = 0; zi < zones.length; ++zi)
			{
				...
				for (int g = 0; g < zone.groupCount(); ++g)
				{
					...
					// Water is kept apart so the opaque pass skips it and the water pass draws it blended.
	...
	public void submit(FrameParams params, GeometryBuffer dynamic, GeometryBuffer translucent, GeometryBuffer water, boolean waitForGl, boolean signalGl)
	{
		...
		// The previous frame's read of the dynamic buffers is done, so refill them. The opaque dynamic
		// set draws in the opaque pass; the per-frame water set draws in the blended water pass below.
		// Translucent geometry still waits for its own blend pass.
		...
		try (MemoryStack stack = stackPush())
		{
			...
			if (dynamicFaceCount > 0 || waterFaceCount > 0)
			{
			...
			if (staticFaceCount > 0)
			{
				...
				for (int i = 0; i < groupCount; ++i)
				{
					...
					// Water is left to the blended water pass; the opaque pass skips it so it is not drawn twice.
					boolean visible = !groupWater[i] && (v == null || groupVisible(v, groupLevel[i], groupRoof[i]));
	...
	@Override public String passReport() { return "normal: raster " + staticFaceCount + "+" + dynamicFaceCount + " faces, water " + waterFaceCount; }
